...
title M-LLM Windows AI Workbench - Full Test R3

rem ============================================================
rem M-LLM Windows AI Workbench - FULL TEST R3
rem Exact source pinned to a verified GUI-fix checkpoint.
rem Default:
rem   download/reuse -> bootstrap -> physical preflight
rem   -> GUI dashboard preflight -> GUI
rem Safety:
rem   OFFLINE_CACHE + isolated runtime data; no Core install authorization.
rem ============================================================

set "COMMIT=229ab18a01e2c13afe4eb3169222e48f843db9b9"
set "MODE=gui"
set "REFRESH=0"
set "NOPAUSE=0"
...
echo.
echo ============================================================
echo  M-LLM Windows AI Workbench - FULL TEST R3
echo ============================================================
echo  Commit : %COMMIT%
echo  Mode   : %MODE%
echo  Policy : NON-INSTALLING PREFLIGHT + GUI PREFLIGHT + OFFLINE_CACHE
echo ============================================================
echo.
...
set "ROOT=%BASE%\M_LLM_WORKBENCH_FULL_TEST_R3"
set "SRC=%ROOT%\source"
...
echo.
echo [2/4] NON-INSTALLING physical preflight...
...
echo.
echo [3/4] NON-INSTALLING GUI dashboard preflight...
...
echo.
echo [4/4] Starting %MODE% in isolated OFFLINE_CACHE mode...
...
:help
echo.
echo M_LLM_WORKBENCH_FULL_TEST_R3.cmd
echo.
echo Double-click:
echo   exact source - bootstrap - physical preflight - GUI dashboard preflight - GUI.
echo.
echo Options:
echo   --gui                 Full safe test then GUI ^(default^)
echo   --doctor              Both preflights then Doctor
echo   --cli                 Both preflights then CLI
echo   --preflight-only      Only physical preflight
echo   --gui-preflight-only  Physical + real GUI Dashboard preflight, no GUI window
