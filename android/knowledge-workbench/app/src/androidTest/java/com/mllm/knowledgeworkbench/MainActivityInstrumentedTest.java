...
public class MainActivityInstrumentedTest {
    ...
    public void bundled_sample_can_build_vectors_and_return_hybrid_evidence() {
        try (ActivityScenario<MainActivity> ignored = ActivityScenario.launch(MainActivity.class)) {
            ...
            onView(withId(R.id.status_hybrid)).check(matches(withText(containsString("可用"))));
            onView(withId(R.id.text_index_coverage)).check(matches(withText(containsString("/"))));
            ...
            onView(withId(R.id.results_container)).check(matches(isDisplayed()));
            onView(withId(R.id.evidence_detail)).check(matches(withText(containsString("车辆"))));
        }
    }
}
