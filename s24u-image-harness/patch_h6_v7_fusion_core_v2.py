...
''',
    ...
    text = replace_once(
        ...
        '''      std::vector<float> fusion_effective_weights(conds.size(), 0.0f);
...
def main() -> int:
    if len(sys.argv) != 2:
        print('usage: patch_h6_v7_fusion_core_v2.py <h6r3-task2-root>', file=sys.stderr)
        return 2
    root = Path(sys.argv[1]).resolve()
    patch_pipeline(root)
    patch_main(root)
    print('S24U_IMAGE_HARNESS_H6R3_FUSION_CORE_V2_APPLIED')
    return 0
