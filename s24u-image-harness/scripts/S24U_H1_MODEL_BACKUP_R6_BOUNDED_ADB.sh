...
VERSION="R6-OFFLINE-PROOT-ADB-BOUNDED"
...
ADB_SOCKET="/data/data/com.termux/files/s24u_image_harness_adb_${$}.sock"
DOWNLOADS_DIR="${HOME}/storage/downloads"
LOG_DIR="${DOWNLOADS_DIR}/S24U_H1_R6_LOGS"
...
select_local_root_wrapper() {
  if command -v proot >/dev/null 2>&1; then
    ROOT_MODE="proot-0"
    ROOT_WRAPPER=(proot -0)
    return 0
  fi
  if command -v fakeroot >/dev/null 2>&1; then
    ROOT_MODE="fakeroot"
    ROOT_WRAPPER=(fakeroot)
    return 0
  fi
  ROOT_MODE="plain-adb"
  ROOT_WRAPPER=()
...
ensure_local_tools() {
  ...
  [[ -n "$ADB_BIN" && -x "$ADB_BIN" ]] || fail "ADB_NOT_INSTALLED_R3_ALREADY_SHOULD_HAVE_INSTALLED_IT"
  select_local_root_wrapper
  reuse_existing_adb_identity
  printf 'ADB_BIN=%s\nROOT_MODE=%s\n' "$ADB_BIN" "$ROOT_MODE" | tee "$LOG_DIR/local-tools.txt"
}
...
adb_samsung() {
  env \
    ...
    "${ROOT_WRAPPER[@]}" "$ADB_BIN" "$@"
}

adb_bounded() {
  local seconds="$1"; shift
  timeout -k 2s "${seconds}s" env \
    HOME="$ADB_HOME" \
    ANDROID_NO_USE_FWMARK_CLIENT=1 \
    ADB_SERVER_SOCKET="localfilesystem:${ADB_SOCKET}" \
    "${ROOT_WRAPPER[@]}" "$ADB_BIN" "$@"
...
cleanup_adb() {
  if [[ -n "$ADB_BIN" ]]; then
    # Never let cleanup hang the terminal. The private socket is per-run, so
    # a failed kill cannot block a future run.
    adb_bounded 3 kill-server >>"$LOG_DIR/adb-server.log" 2>&1 || true
  fi
...
start_private_adb_server() {
  # R5 could block forever here because it called an unbounded
  # `adb ... kill-server` before the first Step-3 checkpoint. R6 uses a
  # unique per-run ADB server socket, so no pre-kill is required.
  rm -f "$ADB_SOCKET" 2>/dev/null || true
  printf '[ADB-R6][3a] starting private ADB server socket=%s timeout=10s\n' "$ADB_SOCKET"
  set +e
  adb_bounded 10 start-server >"$LOG_DIR/adb-server-start.log" 2>&1
  local rc=$?
  set -e
  cat "$LOG_DIR/adb-server-start.log"
  if ((rc == 124 || rc == 137)); then
    fail "ADB_SERVER_START_TIMEOUT"
  fi
  ((rc == 0)) || fail "ADB_SERVER_START_FAILED_RC_${rc}"
  printf '[ADB-R6][3b] start-server returned rc=0; probing version timeout=5s\n'
  set +e
  adb_bounded 5 version >"$LOG_DIR/adb-version.log" 2>&1
  rc=$?
  set -e
  cat "$LOG_DIR/adb-version.log"
  if ((rc == 124 || rc == 137)); then
    fail "ADB_VERSION_PROBE_TIMEOUT"
  fi
  ((rc == 0)) || fail "ADB_VERSION_PROBE_FAILED_RC_${rc}"
  printf '[ADB-R6][3c] private ADB server probe PASS\n'
}
...
verify_shell_uid() {
  ...
  printf '%s\n' "$output" | tee -a "$LOG_DIR/adb-shell-uid.log"
  uid="$(printf '%s\n' "$output" | tr -dc '0-9' | head -c 8)"
  [[ "$uid" == "2000" ]]
}

try_existing_authorization() {
  local ep=""
  ...
  printf '\nR6 不联网安装任何包。请输入无线调试主页面显示的完整“IP 地址和端口”，例如 172.19.0.1:35653：\n> ' >&2
  IFS= read -r -t 300 ep || return 10
  valid_endpoint "$ep" || return 10
  if connect_real_endpoint "$ep" && verify_shell_uid; then
    printf 'ADB_CONNECTION=existing_authorization_manual_endpoint\n'
    return 0
  fi
  ADB_SERIAL=""
  return 10
...
pair_once() {
  ...
  if [[ -z "$connect_ep" ]]; then
    printf '\n配对成功。回到无线调试主页面，只输入“IP 地址和端口”冒号后的连接端口，例如 35653：\n> ' >&2
  ...
  printf 'ADB_CONNECTION=new_pairing_offline_proot\n'
}
...
main() {
  ...
  ensure_local_tools
  progress 2 offline_tools adb_and_local_uid_wrapper PASS

  start_private_adb_server
  progress 3 adb_server private_socket_fwmark_bypass PASS

  progress 4 adb_auth existing_authorization RUN
  if try_existing_authorization; then
    progress 4 adb_auth existing_authorization PASS
  else
    progress 4 adb_auth pairing_required RUN
    pair_once
    progress 4 adb_auth fresh_pairing PASS
  fi
