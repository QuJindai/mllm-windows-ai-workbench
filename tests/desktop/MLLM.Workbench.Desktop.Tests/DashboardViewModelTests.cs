...
public sealed class DashboardViewModelTests
{
    ...
    public async Task Refresh_maps_only_typed_backend_snapshot_into_dashboard_state()
    {
        var snapshot = new DashboardSnapshot(
            new MachineSnapshot("Windows 11 Pro 23H2", "AMD64", "Test CPU", 31.75, ["Test GPU"], 412.6),
            "OFFLINE_CACHE",
            [
                new ComponentSnapshot("git", ComponentHealth.Pass, "git ready", false, null),
                new ComponentSnapshot("local-api", ComponentHealth.Running, "API running", false, null),
                new ComponentSnapshot("python", ComponentHealth.ReadyToInstall, "Python not installed", true, "python"),
                new ComponentSnapshot("modelscope", ComponentHealth.Blocked, "Python required", true, "python")
            ],
            "Qwen3.5-4B Q4_K_M");
        ...
        Assert.Equal("Windows 11 Pro 23H2", vm.OperatingSystem);
        Assert.Equal("Test CPU", vm.Cpu);
        Assert.Equal("31.8 GB", vm.Ram);
        Assert.Equal("Test GPU", vm.Gpu);
        Assert.Equal("412.6 GB", vm.DiskFree);
        Assert.Equal("Qwen3.5-4B Q4_K_M", vm.CurrentModelDisplay);
        Assert.Equal(1, vm.PassCount);
        Assert.Equal(1, vm.RunningCount);
        Assert.Equal(1, vm.ReadyCount);
        Assert.Equal(1, vm.BlockedCount);
        Assert.Single(vm.ServiceComponents);
    ...
    public async Task Backend_failure_is_a_page_error_not_a_component_failure()
    {
        ...
        Assert.Contains("scope failure", vm.BackendError, StringComparison.OrdinalIgnoreCase);
        Assert.Empty(vm.Components);
        Assert.Equal(0, vm.BlockedCount);
    }

    [Fact]
    public void Quick_navigation_commands_emit_approved_phase_a_routes()
    {
        await using var backend = new FakeBackendClient(new DashboardSnapshot(
            new MachineSnapshot("Windows", "AMD64", "CPU", 1, [], 1), "OFFLINE_CACHE", [], null));
        var vm = new DashboardPageViewModel(backend);
        string? route = null;
        vm.NavigationRequested += value => route = value;

        vm.OpenDoctorCommand.Execute(null);
        Assert.Equal("doctor", route);
        vm.OpenInstallationCommand.Execute(null);
        Assert.Equal("installation", route);
    }
    ...
    private sealed class FakeBackendClient : IWorkbenchBackendClient
    {
        ...
        public FakeBackendClient(DashboardSnapshot snapshot) => _snapshot = snapshot;
        ...
        public Task<BackendHandshakeResponse> ConnectAsync(CancellationToken cancellationToken) =>
            Task.FromResult(new BackendHandshakeResponse(true, RpcProtocol.Version, "test", null));
        ...
        public Task<DashboardSnapshot> GetDashboardAsync(CancellationToken cancellationToken) =>
            _error is not null ? Task.FromException<DashboardSnapshot>(_error) : Task.FromResult(_snapshot!);
